...
import { Bell, Check, CheckCheck, X, Trash2, Loader2, Calendar } from 'lucide-react';
        ...
        <div className="min-h-screen bg-gray-50 py-24">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    ...
                    <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex items-center gap-4 animate-fadeIn">
                        <div className={`p-3 rounded-full ${notificationPermission === 'granted' ? 'bg-rose-100 text-rose-600' : 'bg-gray-100 text-gray-500'}`}>
                        ...
                        {notificationPermission !== 'granted' && (
                            <button
                                ...
                                className="ml-auto text-xs font-semibold bg-rose-600 text-white px-3 py-2 rounded-lg hover:bg-rose-700 transition shadow-sm hover:shadow"
                            >
                                Enable
                            </button>
                        )}
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-2 mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    {notifications.some(n => !n.read) && (
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-rose-600 hover:bg-rose-50 rounded-xl transition-colors w-full sm:w-auto justify-center"
                        >
                ...
                {filteredNotifications.length === 0 ? (
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-16 text-center animate-fadeIn">
                    ...
                    <div className="space-y-3">
                        {filteredNotifications.map((notification) => (
                            <div
                                ...
                                className={`group relative bg-white rounded-2xl border transition-all duration-200 hover:shadow-md animate-fadeIn ${!notification.read
                                        ? 'border-rose-200 bg-rose-50/10 shadow-sm'
                                        : 'border-gray-100 hover:border-gray-200'
                                    }`}
                            >
                                <div className="p-5 flex items-start gap-4">
                                    <div className={`flex-shrink-0 mt-1 p-2 rounded-xl ${!notification.read ? 'bg-rose-100/50' : 'bg-gray-100'}`}>
                                ...
                                {!notification.read && (
                                    <div className="absolute left-0 top-6 bottom-6 w-1 bg-rose-500 rounded-r-full" />
                                )}
                            </div>
                        ))}
                    </div>
